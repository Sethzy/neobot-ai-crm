...
import { runExtraction, uploadToExtendAI } from "../../src/lib/extend-ai";
import { splitPdf, getPdfPageCount, shouldSkipSplit, getPdfDimensions } from "../../src/lib/pdf-splitter";
import { offsetCitationPages } from "../../src/lib/highlight-utils";
import {
  validateRequest,
  extractToken,
  createSuccessResponse,
  createErrorResponse,
} from "../../src/api/gemini-process";
import {
  prepareFileForGemini,
  deleteGoogleFile,
} from "../../src/lib/gemini-files";
import {
  sendWhatsAppResult,
  formatProcessingResult,
} from "../../src/lib/whatsapp/send-result";
  ...
  try {
    ...
    if (isInternalCall) {
      ...
      if (!SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
        return res.status(500).json({ success: false, error: "Supabase environment is not configured" });
      }
