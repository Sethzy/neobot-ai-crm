...
The simplest option with easy authentication. Thousands are available.
...
- If \`toolsToActivate\` is listed, those tools will be activated automatically after the connection is created.
- Only one connection per toolkit is allowed. If a toolkit is already connected, delete the existing connection before creating a new one.
...
If the user asks what integrations, apps, or services you can connect to, do not try to enumerate a complete list. Indicate that you can connect to many services through pre-built integrations.
