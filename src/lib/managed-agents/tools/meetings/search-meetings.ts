/**
 * search_meetings tool for managed agents.
 *
 * @module lib/managed-agents/tools/meetings/search-meetings
 */
import { z } from "zod";
...
async function transcriptContainsQuery(
  ...
  const transcriptText = typeof data === "string" ? data : await data.text();
...
function buildMeetingsQuery(
  ...
  let queryBuilder = supabase
    ...
    .select(
      "meeting_record_id, title, summary, duration_seconds, notes, created_at, status, transcript_path",
    )
    ...
    while (matchingRows.length < limit) {
      ...
      for (const row of rows) {
        const matchesTranscript =
          matchesInlineFields(row, normalizedQuery) ||
          (await transcriptContainsQuery(
            context.supabase,
            context.clientId,
            row.transcript_path,
            normalizedQuery,
