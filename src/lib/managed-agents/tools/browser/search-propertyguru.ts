/**
 * search_propertyguru tool for managed agents.
 *
 * @module lib/managed-agents/tools/browser/search-propertyguru
 */
import { z } from "zod";
...
const ALLOWED_PROPERTYGURU_HOSTS = new Set(["propertyguru.com.sg", "www.propertyguru.com.sg"]);
...
const propertyGuruRawListingSchema = z.object({
  ...
  price: z.object({ value: z.number().optional(), pretty: z.string().optional() }).optional(),
  psfText: z.string().optional(),
  bedrooms: z.number().optional(),
  bathrooms: z.number().optional(),
  floorArea: z.number().optional(),
  badges: z.array(z.object({ name: z.string().optional(), text: z.string().optional() })).optional(),
  additionalData: z.object({ tenure: z.string().optional(), districtCode: z.string().optional(), districtText: z.string().optional() }).optional(),
  mrt: z.object({ nearbyText: z.string().optional() }).optional(),
  postedOn: z.object({ text: z.string().optional() }).optional(),
  agent: z.object({ name: z.string().optional(), license: z.string().optional(), profileUrl: z.string().optional() }).optional(),
  agency: z.object({ name: z.string().optional() }).optional(),
  thumbnail: z.string().optional(),
  mediaCarousel: z.object({
...
function normalizePropertyGuruListing(listing: PropertyGuruRawListing): PropertyGuruListing | null {
  if (
...
const inputSchema = z
  .object({
    searchQueries: z.array(z.string().trim().min(1, "Search queries cannot be blank")).optional().describe("Free-text PropertyGuru search queries. Only used when startUrls is empty."),
    startUrls: z.array(z.string().url().refine(isAllowedPropertyguruUrl, {
      message: "Must be a PropertyGuru Singapore URL (propertyguru.com.sg)",
    })).optional().describe("Direct PropertyGuru Singapore search result URLs. Overrides query-builder fields when provided."),
    ...
    maxItems: z.number().int().min(MIN_PROVIDER_ITEMS).max(MAX_TOOL_ITEMS).default(MAX_TOOL_ITEMS).optional().describe(`Maximum listings to return. Minimum ${MIN_PROVIDER_ITEMS}, default ${MAX_TOOL_ITEMS}.`),
  })
  .refine(
    (value) => (value.searchQueries?.length ?? 0) > 0 || (value.startUrls?.length ?? 0) > 0,
    { message: "At least one of searchQueries or startUrls is required" },
  );

type SearchPropertyguruInput = z.infer<typeof inputSchema>;

export const searchPropertyGuruTool: ManagedAgentTool<SearchPropertyguruInput> = {
  name: "search_propertyguru",
  description: "Search current public PropertyGuru Singapore listings using queries or direct search result URLs.",
  inputSchema,
  execute: async ({
    searchQueries,
    startUrls,
