/**
 * search_99co tool for managed agents.
 *
 * @module lib/managed-agents/tools/browser/search-99co
 */
import { z } from "zod";
...
const ninetyNineRawListingSchema = z.object({
  ...
  attributes: z
    .object({
      ...
      price: z.object({ value: z.number().optional(), formatted_string: z.string().optional() }).optional(),
      psf: z.object({ formatted_string: z.string().optional() }).optional(),
      beds: z.object({ value: z.union([z.string(), z.number()]).optional() }).optional(),
      bathrooms: z.object({ value: z.number().optional() }).optional(),
      floorarea_sqft: z.object({ value: z.number().optional() }).optional(),
  ...
  commute_nearest_mrt: z
    ...
    .optional(),
  agent: z.object({ name: z.string().optional(), phone: z.string().optional(), whatsapp: z.string().optional() }).optional(),
...
function buildApiUrl(searchUrl: string, options: { pageNumber: number; pageSize: number }): string {
...
function normalize99coListing(listing: NinetyNineRawListing): NinetyNineListing | null {
  ...
  const builtYear = listing.attributes?.top ? Number.parseInt(listing.attributes.top, 10) : undefined;
...
const inputSchema = z.object({
  ...
  maxItems: z.number().int().min(1).max(MAX_TOOL_ITEMS).default(MAX_TOOL_ITEMS).optional().describe(`Maximum listings to return. Default ${MAX_TOOL_ITEMS}.`),
});

type Search99coInput = z.infer<typeof inputSchema>;

export const search99coTool: ManagedAgentTool<Search99coInput> = {
  name: "search_99co",
  description: "Search current public 99.co Singapore listings using one or more 99.co search URLs.",
  inputSchema,
