...
const boardCardClassName =
  "group rounded-md border border-app-border-subtle bg-app-surface p-3 transition-all duration-[var(--duration-hover)] hover:bg-app-hover/35 hover:shadow-xs";
    ...
    >
      <div className="flex items-center gap-2 px-2 pb-3">
        {column.toneClassName ? (
          <span
            className={cn(
              "inline-flex rounded-[var(--radius-chip)] px-1.5 py-0 text-caption font-medium",
              column.toneClassName,
            )}
          >
            {column.label}
...
function KanbanDragOverlay<T>({ activeItem, renderCard }: KanbanDragOverlayProps<T>) {
  return (
    <DragOverlay>
      {activeItem ? (
        <KanbanCardShell className="shadow-sm">
