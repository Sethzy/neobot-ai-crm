 ...
 * Uses ShadCN Sheet for mobile nav drawer. Resources dropdown for property data pages.
 */
 'use client'
...
const RESOURCE_LINKS = [
  { href: '/agents', label: 'Agent Profiles', description: 'CEA agent transaction history' },
  { href: '/properties', label: 'Private Properties', description: 'Condo & residential projects' },
  { href: '/hdb', label: 'HDB Resale', description: 'HDB resale streets & pricing' },
  { href: '/agencies', label: 'Agencies', description: 'Agency activity & top agents' },
  { href: '/areas', label: 'Areas', description: 'Town & district transactions' },
] as const
...
/** Desktop dropdown for Resources links. */
  ...
  return (
    ...
    >
      <button
        ...
        className="inline-flex items-center gap-1 text-sm font-medium text-zinc-600 transition hover:text-sunder-green"
      >
        Resources
      ...
      {isOpen ? (
        <div className="absolute left-1/2 top-full z-50 mt-2 w-64 -translate-x-1/2 rounded-xl border border-zinc-200/80 bg-white p-2 shadow-lg shadow-zinc-900/[0.08] ring-1 ring-zinc-900/[0.04]">
          {RESOURCE_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              onClick={() => setIsOpen(false)}
              className="block rounded-lg px-3 py-2.5 transition hover:bg-zinc-50"
            >
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent side="top" showCloseButton={false} className="rounded-b-2xl p-4">
        ...
        <nav className="flex flex-col text-lg tracking-tight text-foreground">
          ...
          <p className="px-2 pt-1 pb-1 text-xs font-semibold uppercase tracking-wider text-zinc-400">
            Resources
          </p>
          {RESOURCE_LINKS.map((link) => (
