...
async function fetchPropertyProfile(
  ...
  const recentTransactionsBaseQuery = client
    .from("ura_transactions")
    .select(
      "contract_date, price, price_psf, area_sqm, floor_range, type_of_sale, property_type, tenure, no_of_units"
    )
    .eq("project", context.project)
    .order("contract_date", { ascending: false })
    .limit(500);
      ...
      <section className="py-12 sm:py-16">
        <Container>
          ...
          <div className="mt-6 grid gap-4 grid-cols-2 sm:grid-cols-3 xl:grid-cols-5">
            ...
            <StatCard
              label="Avg PSF"
              value={formatCurrencySgd(profile.averagePsf)}
            />
            <StatCard
              label="Median Price"
              value={formatCurrencySgd(profile.medianPrice)}
            />
            <StatCard
              label="Price Range"
              value={formatPriceRange(profile.priceRange.min, profile.priceRange.max)}
            />
            <StatCard
              label="Last Sale"
