...
async function fetchAgencySummaryAndTopAgents(
  ...
  return { transactionCount, latestTransactionDate, topAgents };
}
...
export default async function AgencyProfilePage({
  ...
  const { transactionCount, latestTransactionDate, topAgents } =
      ...
      <section className="py-12 sm:py-16">
        <Container>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <StatCard label="Active Agents" value={formatCount(agents.length)} />
            <StatCard
            ...
            <StatCard
              ...
              value={formatDateMonthYear(latestTransactionDate)}
            />
            <StatCard
              label="Top Agents Shown"
              value={formatCount(topAgents.length)}
            />
          </div>
        </Container>
      </section>
                ...
                {topAgents.map((agent) => (
                  <tr key={agent.registrationNo} className="hover:bg-zinc-50/80">
                    <td className="px-4 py-4 text-sm text-zinc-900">
                      <Link
                        href={`/agents/${agent.registrationNo}`}
