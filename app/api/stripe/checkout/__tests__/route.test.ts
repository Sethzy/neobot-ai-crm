...
describe("GET /api/stripe/checkout", () => {
  ...
  it("redirects to /settings/workspace/billing after a successful fallback sync", async () => {
    const { GET } = await import("../route");
    ...
    expect(response.status).toBe(307);
    expect(response.headers.get("location")).toBe("http://localhost/settings/workspace/billing?billing=success");
  });
