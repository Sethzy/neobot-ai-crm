...
```
[Screenshot: A Slack message invoking a minion run]
```
...
```
[Screenshot: A flaky test ticket with a button to start a minion that'd fix it]
```

While the minion works, or after the fact, engineers can see the decisions and actions the minion took in a web UI.

```
[Screenshot: An example of the web interface for managing minion runs]
```
...
### Isolated Developer Environments (Devboxes)
...
## Key Architecture Diagram (Reconstructed)

```
ENTRY POINTS
+-------------+  +-------------+  +-------------+  +----------------+
|   Slack     |  |    CLI      |  |   Web UI    |  | Internal Apps  |
| (@minion)   |  |             |  |             |  | (docs, flags,  |
|             |  |             |  |             |  |  tickets)      |
+------+------+  +------+------+  +------+------+  +-------+-------+
       |                |                |                  |
       +--------+-------+--------+-------+--------+--------+
        ...
        |                                                  |
        |  1. Spin up pre-warmed devbox (10 sec)           |
        |  2. Hydrate context (MCP tools on links)         |
        |  3. Load conditional agent rules                 |
        |  4. Run agent loop (Goose fork)                  |
        |     - Interleave agent + deterministic steps     |
        |  5. Local lint/test (< 5 sec)                    |
        |  6. Push to CI                                   |
        |  7. If fail: fix + push once more (max 2 CI)     |
        |  8. Create PR following template                 |
        +---+------------------------------------------+---+
            |                                          |
            v                                          v
   +--------+--------+                    +------------+-----------+
   |   TOOLSHED      |                    |   DEVBOX (isolated)    |
   |   (MCP Server)  |                    |                        |
   |   400+ tools    |                    |  - Stripe code         |
   |                  |                    |  - Services pre-loaded |
   |   - Docs         |                    |  - No internet         |
   |   - Tickets      |                    |  - No prod access      |
   |   - Sourcegraph  |                    |  - Git worktree        |
   |   - Build status |                    |                        |
   |   - SaaS tools   |                    +------------------------+
   +------------------+
            |
...
1. **Pre-warmed isolated environments (devboxes) are critical.** 10-second spin-up, code pre-loaded, isolated from prod/internet. Same pattern as Fly Sprites but enterprise-grade.

2. **Deterministic steps interleaved with agent creativity.** Don't let the agent decide whether to lint or test -- force it. Mix deterministic code with agent loops.

3. **"Shift feedback left."** Run lints locally in 5 seconds before ever hitting CI. Saves tokens, compute, time.

4. **At most two CI rounds.** Diminishing returns on repeated CI loops. Fix locally first, then one CI run, one retry, done.

5. **Conditional agent rules by subdirectory.** Unconditional rules don't scale in large codebases.

6. **Toolshed (centralized MCP server).** 400+ tools in one place, curated subsets per agent. Common language across all agents.

7. **Pre-hydrate context deterministically.** Run MCP tools on links BEFORE the agent starts, not during.

8. **Multiple entry points.** Slack, CLI, web, internal apps. Meet engineers where they are.

9. **1000+ PRs/week with no human-written code.** Scale metric for unattended agents.

10. **Git worktrees don't scale at Stripe's size.** They use full isolated devboxes instead. Elvis uses worktrees because his codebase is small enough.
