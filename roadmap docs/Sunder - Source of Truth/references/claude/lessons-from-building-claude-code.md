...
- **A computer** would be the fastest and most powerful option, but you would have to know how to use it to write and execute code
...
```
  ...
  <────────────────────────o───────────────────────────────>
         |                 |                    |
  ┌──────────────┐  ┌─────────────────┐  ┌──────────────┐
  │  modified     │  │ AskUserQuestion │  │ ExitPlanTool │
  │  markdown     │  │     tool        │  │  parameter   │
  │  output       │  │                 │  │              │
  └──────────────┘  └─────────────────┘  └──────────────┘
   model free but     structured +         plan already
   messy, hard to     composable,          formed,
   format             clear UI surface     questions come
                                           too late
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│ ← □ Paradigm  □ Theme  □ Snacks  □ Indentation  ✓ Submit  →    │
│                                                                 │
│ Which programming paradigm do you prefer for this project?      │
│                                                                 │
│ > 1. Functional                                                 │
│      Pure functions, immutability, composition over inheritance  │
│   2. Object-Oriented                                            │
│      Classes, encapsulation, inheritance hierarchies            │
│   3. Procedural                                                 │
│      Straightforward step-by-step functions and state           │
│   4. Mixed                                                      │
│      Whatever fits the problem best, no strict rules            │
│   5. Type something.                                            │
│                                                                 │
│   6. Chat about this                                            │
│                                                                 │
...
```
       ...
       TODOS                              TASKS
  ┌──────────────┐                 ┌─────────┐ ┌─────────┐
  │ Single Agent │                 │ Agent A │ │ Agent B │
  └──────┬───────┘                 └────┬────┘ └────┬────┘
         │                              │           │
  ┌──────────────┐      models    ┌─────┴───┐ ┌────┴────┐
  │☑ Set up      │    ────────>   │ Task 1 ✓│ │ Task 2 ✓│
  │  project     │     improve    └─────────┘ └─────────┘
  ├──────────────┤                      ┌─────────┐
  │☑ Write tests │                      │ Task 3 ●│
  ├──────────────┤                      └────┬────┘
...
**Key insight:** As Claude gets smarter, it becomes increasingly good at building its context if it's given the right tools.
...
1. **Tool design = model affordances** — shape tools to the model's abilities, not yours
2. **Progressive disclosure > fat system prompts** — let agents discover context on demand
3. **Revisit tool assumptions as models improve** — what helped before may now constrain
4. **Structured tools > free-text output** — AskUserQuestion beats parsed markdown
5. **Subagents protect context** — delegate search/exploration to keep main context clean
6. **The bar for new tools is high** — prefer progressive disclosure over adding tools
