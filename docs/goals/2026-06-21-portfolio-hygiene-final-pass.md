...
| Retained workbench folders look intentional | README/docs explain `.agents/`, `.claude/`, `.codex/`, `.context/`, `.kiro/`, `.windsurf/`, `skills/`, and `skills-lock.json`, or local VS Code hides ignored noise | Verified | `README.md` Agent Workbench section names the retained workbench artifacts and separates them from `src/lib/managed-agents/`, `managed-agents/skills/`, and `scripts/managed-agents/`. |
