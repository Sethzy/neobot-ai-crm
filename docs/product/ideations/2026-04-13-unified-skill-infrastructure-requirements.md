...
- R10. A single API endpoint returns the merged skill list: predefined skill metadata (name + description from SKILL.md frontmatter) combined with user-customized skill slugs from Supabase storage. The frontend calls this on composer mount to populate the autocomplete.
- R11. User-created skills (net new skills in the user's Supabase storage, not overrides of predefined ones) appear in the autocomplete alongside predefined skills.
...
- Adding a new predefined skill requires only: create `managed-agents/skills/{slug}/SKILL.md`, run the upload pipeline, redeploy — no frontend code change needed
...
- **One API endpoint for skill list:** Merges predefined skills (from SKILL.md frontmatter) + user overrides/creations (from Supabase storage). No new DB table, no Anthropic API call at runtime. KISS.
...
- [Affects R10][Technical] Should the skill discovery endpoint be a Server Action, an API route, or a React Server Component data fetch? What caching strategy?
