...
**Source:** @elvissun on X (Article), 2026-02-23
**URL:** https://x.com/elvissun (Article post)
**Views:** 5.2M
...
## Why This Works Better Than Using Codex or Claude Code Directly
...
+---------------------+              | ENGINEERING DOCS    |
| SKILLS              |              | * Design docs        |
| * Marketing          |              | * Feature specs      |
| * Research           |              | * API schemas        |
| * Writing            |              |                     |
| * Web search         |              +---------------------+
+---------------------+              | CODEBASE            |
| MEMORY SYSTEM       |              | * src/components/    |
| * MEMORY.md          |              | * src/lib/           |
| * Daily notes        |              | * Type definitions   |
| * Past decisions     |              | * Test patterns      |
+---------------------+              +---------------------+
| (minimal code)      |              | (just task prompt)  |
+---------------------+              +---------------------+
...
```
+-------------------+  +-------------------+  +--------+  +------------------+
| Sentry Error Logs |  | Support Tickets   |  |   Me   |  | Meeting notes    |
+--------+----------+  +--------+----------+  +---+----+  +--------+---------+
         |                       |                 |                |
         +----------+------------+---------+-------+--------+------+
                    |                      |                |
              ...
              +------------+---------------------------+--------+
              | Github CI/CD                                    |
              | (Lint -> typecheck -> tests -> e2e -> AI x3)    |
              +------------------------+-----------------------+
                                       |
                              +--------v--------+
                              |     Pass?       |<---- Cron job every 10 min
                              +---+----------+--+
                                  |          |
                                No|          |Yes
                                  |          |
...
## Andrej Karpathy Response

> "Can't tell if brilliant or severe AI psychosis nice" -- @karpathy, Feb 24

---

## Key Takeaways for Sunder

1. **Two-tier context separation is the core insight.** Business context in orchestrator, code context in coding agent. Same pattern as Sunder's outer agent (CRM/memory/skills) vs inner agent (Claude CLI in Sprite).

2. **Git worktree per agent for isolation.** Each agent gets its own branch and working directory. Sunder uses per-thread Sprites for the same purpose.

3. **Deterministic monitoring over polling agents.** The cron checks git/CI status, not the agent itself. Token-efficient. Sunder's cron fallback for async sandbox follows this pattern.

4. **Definition of done matters.** PR + CI + 3x AI review + screenshots. Not just "agent finished."

5. **Mid-task redirection via tmux.** Ability to course-correct without killing the agent. Sunder doesn't have this yet for sandbox jobs.

6. **RAM is the real bottleneck** for local multi-agent setups. Each worktree + node_modules + compiler is heavy.

7. **Ralph Loop V2** -- retry with richer context, not same prompt. Zoe adapts prompts based on failure reason + business context.

8. **Agent routing by strength.** Codex for backend/reasoning, Claude Code for frontend/git, Gemini for design. Sunder currently uses one model for sandbox.
