# Why We Built Our Own Background Agent (Ramp Inspect)
...
The key challenge is spinning up full dev environments quickly. Modal solves this: it's a cloud platform for AI infrastructure we use across Ramp. Their sandboxes start near instantly, and file system snapshots allow us to freeze and restore state later. With Modal Sandboxes, we take the following approach:
...
You now need to set up an agent to do work in the sandbox. We strongly recommend using OpenCode, an open source coding agent. We believe it has critical advantages over other agents:
...
- OpenCode was the strongest technical implementation we found, as it has a fully typed SDK, and comprehensive plugin system
...
- If you build something great, you will get the OpenCode team to work with you
...
- Move as much as you can to the build step for your repository images
  - Thoroughly investigate what you need to do to have a complete development environment, and do literally everything you can beforehand
...
The best system we found for this was Cloudflare's Durable Objects. Every session gets its own SQLite database. This ensures high performance, even when you have hundreds of sessions running in parallel, as no one session can impact another. This is particularly important given that the agent will be streaming tokens to you in real time, so you will likely receive hundreds of updates in a very short time frame.

Cloudflare also provides its Agents SDK, which we found was particularly useful for handling the real-time streaming between the sandbox, the API, and any attached clients. It provides helpful abstractions over the WebSockets Hibernation API, which allows for sockets to stay open without incurring compute costs during idle periods.
...
- A hosted VS Code instance, which runs inside of the sandbox, so people can make manual changes without having to clone the repository locally
...
+----+-----+  +-----+-----+  +----+-----+  +----+-----+  +----+-----+
     ...
     |                                                         |
     |  - Real-time streaming (Agents SDK + WebSockets)        |
     |  - State sync across all clients                        |
     |  - Multiplayer: multiple users per session              |
     |  - Message queue (follow-ups queued during execution)   |
     +---+---------------------------------------------+-------+
         |                                             |
         v                                             v
     +---+---+                                 +-------+--------+
...
1. User starts typing
   --> Warm sandbox begins spinning up (pre-emptive)

2. User hits enter
   --> Sandbox ready (from 30-min-old snapshot)
   --> git pull latest (at most 30 min of changes)
   --> Agent starts reading files IMMEDIATELY
       (writes blocked until sync complete)

3. Agent works
   --> Streams tokens to all clients via Durable Objects
   --> Follow-up prompts queued, not injected mid-execution
   --> Can spawn sub-sessions for research or parallel PRs

4. Agent finishes
   --> Sandbox pushes branch
   --> API creates PR via user's GitHub token
   --> Snapshot saved for follow-ups

5. Follow-up (hours/days later)
...
1. **Pre-built images rebuilt every 30 minutes.** Don't clone + install at session start. Do it ahead of time. Session start = restore snapshot + sync delta.

2. **Let agent read before sync completes.** Block writes, allow reads. In a large repo, the files being read are unlikely to have changed in the last 30 minutes.

3. **Warm sandboxes on keystroke.** Start spinning up before the user finishes typing. First-token latency should only be model latency.

4. **Queue follow-ups, don't inject mid-execution.** Easier to manage, lets users queue thoughts while agent works.

5. **Agents should spawn agents.** Give the agent a tool to create sub-sessions. Frontier models are smart enough not to abuse it.

6. **Multiplayer is mission-critical.** Multiple people in one session. Free if you already sync state across clients.

7. **OpenCode over other agents.** Server-first architecture, typed SDK, plugin system, readable source code (agent can read its own code to understand itself).

8. **Visual verification for frontend.** Agent takes before/after screenshots, appends to PR. Streamed desktop view for real-time observation.

9. **Merged PRs as the north-star metric.** Not sessions started, not code generated. Merged PRs = valuable work.

10. **30% of all merged PRs at Ramp are Inspect-written.** Reached in just a couple months. Adoption was organic, not mandated.

11. **Chrome extension for non-engineers.** Uses DOM + React internals instead of screenshots (token-efficient). Distributed via MDM policy, not Chrome Web Store.

12. **Cloudflare Durable Objects for per-session state.** Each session gets its own SQLite DB. No cross-session interference. WebSocket hibernation for idle efficiency.
