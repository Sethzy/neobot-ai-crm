...
- TypeScript and Python SDKs (`@vercel/sandbox`)
- Millisecond cold starts
- Snapshotting for resumable sessions (persist up to 30 days)
- Node.js 24/22 and Python 3.13 runtimes with full sudo access
- CLI and SDK for automation
- Available across Hobby, Pro, and Enterprise plans

_Philosophy:_ Vercel treats sandboxes as a compute primitive for safely running untrusted or user-generated code. Designed for AI agents, code generation, and developer experimentation. Integrates natively with Vercel's deployment platform.

_Unique Features:_

- _Active CPU billing_: Only bills when CPU is actively executing — I/O wait (network requests, DB queries, LLM API calls) is free. Agents waiting on model responses don't burn CPU credits.
- _Millisecond cold starts_: Fastest of any Firecracker-based option
- _Snapshotting_: Save and resume sandbox state; skip dependency installation on subsequent runs (30-day expiry default)
- _Native Vercel integration_: OIDC auth, automatic in production on Vercel
- _Pro plan credit_: $20/mo platform fee includes $20 usage credit that covers sandbox usage

_Pricing:_

| Plan       | Monthly Fee | Active CPU          | Memory             | Notes                                     |
| ---------- | ----------- | ------------------- | ------------------ | ----------------------------------------- |
| Hobby      | $0          | 5 hrs included      | 420 GB-hrs included | Hard cap, pauses after limits             |
| Pro        | $20         | $0.128/hr           | $0.0212/GB-hr      | $20 credit covers all usage first         |
| Enterprise | Custom      | Same as Pro (list)  | Same as Pro (list) | Volume discounts available                |

_Additional costs (Pro):_

| Resource          | Rate          | Hobby Included |
| ----------------- | ------------- | -------------- |
| Creations         | $0.60/1M      | 5,000/mo       |
| Network           | $0.15/GB      | 20 GB/mo       |
| Snapshot Storage  | $0.08/GB-mo   | -              |

_Example costs (Pro, 100% CPU utilization):_
...
_Pro plan credit math (2 vCPU / 4GB sandbox):_
- ~$0.21/hr at 100% CPU utilization
- $20 credit covers ~95 hrs/mo at full utilization
- Agents spending 70% of time on I/O wait → effectively ~270+ hrs/mo covered
...
| Max vCPUs          | 8 (1, 2, 4, or 8 configurable)              |
| Memory per vCPU    | 2 GB (fixed ratio)                           |
| Max Memory         | 16 GB                                        |
| Max Duration       | 45 min (Hobby), 5 hrs (Pro/Enterprise)       |
| Concurrent         | 10 (Hobby), 2,000 (Pro/Enterprise)           |
| Open Ports         | 15 per sandbox                               |
| Region             | iad1 only (currently)                        |
| Snapshot Expiry    | 30 days (configurable)                       |

_Limitations:_

- Max 5-hour session duration (vs 24h on E2B, unlimited on Sprites)
- Single region only (iad1) — no edge distribution
- 2 GB memory per vCPU fixed ratio (can't configure independently)
- Snapshot storage costs extra ($0.08/GB-mo)
- No pre-installed AI CLI tools (unlike Sprites)
- 15 open ports limit per sandbox

_Links:_

- Official: https://vercel.com/docs/sandbox
...
- System Specs: https://vercel.com/docs/vercel-sandbox/system-specifications
...
| Feature          | **Vercel Sandbox**      | Sprites             | E2B                 | Modal                | Cloudflare         |
| ---------------- | ----------------------- | ------------------- | ------------------- | -------------------- | ------------------ |
| Setup            | **Very Easy**           | Easy                | Very Easy           | Easy                 | Easy               |
| Free Tier        | **5 hrs + $20 credit**  | $30 credit          | $100 credit         | $30/month            | $5/mo Workers Paid |
| Isolation        | **Firecracker microVM** | Firecracker microVM | Firecracker microVM | gVisor container     | Container          |
| Cold Start       | **Milliseconds**        | <1 second           | ~150ms              | 2-5s (or <3s w/snap) | 1-5 seconds        |
| Max Timeout      | 5 hours (Pro)           | None (persistent)   | 24 hours (Pro)      | 24 hours             | Configurable       |
| Claude CLI       | Manual                  | Pre-installed       | Prebuilt template   | Manual               | Manual             |
| Git Support      | Yes                     | Yes                 | Yes                 | Yes                  | Yes                |
| Persistent Files | Via Snapshots (30d)     | Yes (permanent)     | 24 hours            | Via Volumes          | Via R2 FUSE mount  |
| Checkpoints      | Snapshots (30d)         | Yes (~300ms)        | Pause/Resume (Beta) | Memory Snapshots     | No                 |
| Network Controls | Default open            | Layer 3 policies    | Allow/deny lists    | CIDR allowlists      | Bindings model     |
| Edge Locations   | iad1 only               | Fly.io regions      | -                   | -                    | 330+ global        |
| Max Concurrent   | 2,000 (Pro)             | 10 active (base)    | Plan-based          | 10,000+              | Plan-based         |
| Self-Hosting     | No                      | Fly.io only         | Experimental        | No                   | No                 |
| MCP Tools        | -                       | -                   | 200+ (Docker)       | -                    | -                  |
| Active CPU Only  | **Yes**                 | No                  | No                  | No                   | No                 |
| Best For         | **Vercel-native apps**  | Long-running agents | AI agent loops      | Python ML workloads  | Edge apps          |

---

## Third-Party Benchmarks

### Mert Devici Benchmark (2026-01-17)

**Source:** https://sandbox-comparison.pages.dev/ (contact: m+sandbox@mertdev.dev)

Real API call benchmarks across 5 providers. **Note: Vercel Sandbox is not included.**

_Performance (seconds):_

| Provider | Cold Start | Cmd Exec | File I/O | Python |
|----------|-----------|----------|----------|--------|
| E2B      | 0.515 BEST | 0.539   | 0.512   | 0.288  |
| Daytona  | 0.753     | 0.975   | 0.386 BEST | 0.220 BEST |
| Sprites  | ~1.0      | 0.970   | 0.957   | 0.543  |
| Modal    | 1.512     | 1.913   | 2.278   | 0.222  |
| Blaxel   | 2.9/25ms  | 0.539   | 0.992   | N/A    |

_Pricing (1 vCPU + 1GB RAM, wall-clock billing):_

| Provider | $/hour | 1 sandbox x 24h | 1000 sandboxes x 1h |
|----------|--------|-----------------|---------------------|
| Sprites  | $0.008 CHEAPEST | $0.20  | $8.20               |
| Blaxel   | $0.041 | $0.99           | $41.40              |
| E2B      | $0.067 | $1.60           | $66.60              |
| Modal    | $0.166 | $3.98           | $166.00             |

_Spin down / scale-to-zero:_

| Provider | Behavior | Resume Time |
|----------|----------|-------------|
| Blaxel   | 1s idle → standby | <25ms resume |
| Modal    | Immediate scale-to-zero | ~1-2s cold start |
| Sprites  | Auto-hibernate | ~1s wake |
| E2B      | Configurable timeout | Default 300s |

_Snapshots:_

| Provider | Support | Details |
|----------|---------|---------|
| Sprites  | Full BEST | ~300ms create, last 5, git-like versioning |
| Modal    | Filesystem | snapshot_filesystem() creates Image |
| Blaxel   | Standby state | Memory preserved during standby |
| E2B      | Templates only | No runtime snapshots |
| Daytona  | None | Use persistent storage |

### Why Sprites Appears Cheapest (and Why It's Apples-to-Oranges with Vercel)

Sprites wins on raw wall-clock $/hr ($0.008/hr) because:
1. Memory is included in CPU price (no separate charge)
2. Auto-hibernate means zero cost when idle
3. Fly.io shared-cpu pricing is very aggressive

**But this benchmark doesn't account for Vercel Sandbox's active-CPU-only billing.** All other providers bill wall-clock time (CPU allocated, whether used or not). Vercel only bills when the CPU is actively executing — I/O wait is free.

For an AI agent that spends ~70% of time waiting on LLM API responses:
- **Sprites wall-clock:** $0.008/hr (full hour billed regardless)
- **Vercel active CPU:** $0.128/hr × 0.30 (30% active) = ~$0.038/hr effective
- **E2B wall-clock:** $0.067/hr (full hour billed)

So Vercel is cheaper than E2B and Modal for I/O-heavy agent workloads, but Sprites still wins on pure $/hr if the sandbox is CPU-bound. For Sunder's use case (agent waiting on LLM calls most of the time), Vercel's billing model is very competitive, plus we're already paying for the Pro plan.
...
## Decision: Vercel Sandbox First

> **Decision date:** 2026-03-17
> **Status:** Active

**Sunder will use Vercel Sandbox as the primary sandbox environment.**

### Rationale

1. **Already on Vercel.** Sunder's frontend and serverless functions deploy on Vercel. Sandbox integrates natively — OIDC auth is automatic in production, no extra infra to manage.
2. **Cost-effective for our workload.** Active-CPU-only billing is ideal for AI agents that spend most of their time waiting on LLM API responses. The $20/mo Pro credit covers significant sandbox usage before any overage.
3. **Fastest cold start.** Millisecond startup means sandbox creation can be part of the request path without latency concerns.
4. **Firecracker isolation.** Same microVM technology as E2B — strong security boundary for running untrusted/agent-generated code.
5. **Snapshots.** Can save and resume sandbox state, skipping dependency installation on repeat runs.
6. **Simplicity.** One vendor for hosting + compute + sandbox. Fewer moving parts, fewer bills, fewer integration points.

### Trade-offs accepted

- **5-hour max session** (vs 24h on E2B, unlimited on Sprites) — acceptable for Sunder's agent runs which are typically minutes, not hours.
- **Single region (iad1)** — acceptable; our Supabase and Vercel deployments are US-based.
- **No pre-installed AI CLI tools** — we don't need them; our agent runs via Vercel AI SDK, not CLI.
- **No granular network controls** — acceptable for v1; our sandbox workloads don't need egress restrictions beyond what the agent's tool permissions already enforce.

### Fallback options

If Vercel Sandbox doesn't meet future needs:
- **E2B** — for 24-hour sessions or multi-language untrusted code execution
- **Sprites** — for persistent, long-lived agent environments that survive across days/weeks
- **Docker Local** — for local development and prototyping

---

## Other Recommendations (Reference)

### For Production/Multi-tenant (Alternative): E2B
...
### For Long-Running Persistent Agents (Alternative): Sprites
...
### For Local Development: Docker Sandboxes
