...
**Source:** https://www.nicolasbustamante.com/p/the-crumbling-workflow-moat-aggregation
...
This isn't incremental change. This is the completion of Ben Thomson's Aggregation Theory.
...
**The value chain was simple: Suppliers → Distributors → Consumers.**
...
1. **Brand persistence**: Users saw the New York Times, not just "a news source." Brand equity survived aggregation.

2. **UX differentiation**: Suppliers could compete on design, speed, features. A better interface meant higher conversion.

3. **Switching costs**: Users developed muscle memory, workflow habits. Learning a new system had real friction.

4. **Monetization control**: Suppliers owned their conversion funnels. They controlled the paywall, the checkout, the subscription flow.
...
```
┌──────────────────────────────────────────┐
│   VERTICAL SOFTWARE VALUE STACK          │
├──────────────────────────────────────────┤
│                                          │
│  Interface Layer (60-80% of value)       │
│  ┌────────────────────────────────────┐  │
...
│  └────────────────────────────────────┘  │
│                                          │
│  Result: 20-30x earnings multiples       │
│                                          │
└──────────────────────────────────────────┘
```
...
OLD:  Discovery → Interface → Data
      [Google]   [Supplier]  [Supplier]

NEW:  LLM Chat → APIs
      [ChatGPT]  [Invisible Suppliers]
```
...
│  WEB 2.0 (Before)                       │
│  ┌──────────┐                           │
│  │  Google  │  ← User sees this         │
│  └────┬─────┘                           │
│       │                                 │
│       ▼                                 │
│  ┌──────────┐                           │
│  │ Supplier │  ← User VISITS this       │
│  │ Website  │  ← User SEES brand        │
│  │          │  ← User USES interface    │
│  └──────────┘                           │
│                                         │
│  LLM ERA (After)                        │
│  ┌──────────┐                           │
│  │ LLM Chat │  ← User sees ONLY this    │
│  └────┬─────┘                           │
│       │ (invisible)                     │
│       ▼                                 │
│  ┌──────────┐                           │
│  │   API    │  ← Supplier is invisible  │
│  │ Backend  │  ← No brand               │
│  │  Data    │  ← No interface           │
│  └──────────┘                           │
│                                         │
...
├───────────────────────────────────────────┤
│                                           │
│  Vertical SaaS (2020): $20B valuation    │
│  ┌─────────────────────────────────────┐ │
│  │  Interface Value: 60% = $12B        │ │
│  │  ════════════════════════            │ │
│  │                                      │ │
│  │  Data Value: 40% = $8B               │ │
│  │  ════════════                        │ │
│  └─────────────────────────────────────┘ │
│                                           │
│  After LLM Absorption (2026)              │
│  ┌─────────────────────────────────────┐ │
│  │  Interface Value: 0% = $0           │ │
│  │  (LLMs own the interface)            │ │
│  │                                      │ │
│  │  Data Value: 40% = $8B               │ │
│  │  ════════════                        │ │
│  │                                      │ │
│  │  - But if data is commoditized:      │ │
│  │    $8B → $2B (commodity pricing)     │ │
│  └─────────────────────────────────────┘ │
│                                           │
│  Result: $20B → $2B valuation            │
│                                           │
└───────────────────────────────────────────┘
```
...
## Reservation API:
POST /book {date, time, party_size}
```
...
│  │  Switching = engineering project   │  │
│  │  Moat = integration effort          │  │
│  └────────────────────────────────────┘  │
│                                          │
│  NEW: MCP                                │
│  ┌────────────────────────────────────┐  │
│  │  Standardized protocol             │  │
│  │  LLM understands natively          │  │
│  │  Switching = instant               │  │
│  │  No moat                            │  │
│  └────────────────────────────────────┘  │
│                                          │
...
├───────────────────────────────────────────┤
│                                           │
│  Web 2.0 Switching Costs                  │
│  ┌─────────────────────────────────────┐ │
│  │  Interface Retraining:    $50,000   │ │
│  │  Workflow Rebuild:        $30,000   │ │
...
│  └─────────────────────────────────────┘ │
│                                           │
│  LLM Era Switching Costs                  │
│  ┌─────────────────────────────────────┐ │
│  │  LLM switches API:        $0        │ │
│  │  User sees no difference: $0        │ │
│  │  Integration work:        $0        │ │
│  │  ─────────────────────────────────  │ │
│  │  TOTAL:                   $0        │ │
│  └─────────────────────────────────────┘ │
│                                           │
│  Result: Price/Quality competition only   │
│                                           │
└───────────────────────────────────────────┘
```
...
│  │        [LLM AGGREGATOR]              │ │
│  │        (THICK LAYER)                 │ │
│  │    Discovery + Interface + UX        │ │
│  │               │                      │ │
│  │               ▼                      │ │
│  │            APIs                      │ │
│  │      (Thin commodity layer)          │ │
│  │      (Invisible to users)            │ │
│  └─────────────────────────────────────┘ │
│                                           │
└───────────────────────────────────────────┘
...
│           └─────────┴─────────┘           │
│                                           │
│  SAFE: Bloomberg (proprietary + network)  │
│  SURVIVE: Data vendors with unique sets   │
│  RISKY: Vertical SaaS (commoditized data) │
│  EXTINCT: Workflow tools (no data moat)   │
│                                           │
└───────────────────────────────────────────┘
```
...
1. **LLM Chat Interface Owners**: Whoever owns the chat interface owns the user relationship. OpenAI with ChatGPT. Anthropic with Claude. Microsoft with Copilot. Google with Gemini. They capture the interface value that vertical software loses. **The new aggregators.**

2. **Proprietary Data Owners**: Companies with truly unique, non-replicable data. The key test: Can this data be licensed or scraped? If yes, not defensible. If no, you survive.

3. **MCP-First Startups**: Companies building for agents, not humans. No legacy interface to protect. No beautiful UI to maintain. Just clean data served through MCP endpoints that LLMs can query. They can undercut incumbents on price because they have no interface investment to recoup.

### The Losers:

1. **Interface-Moat Businesses**: Any vertical software where "workflow" was the value. The interface that justified premium pricing becomes worthless. A $20B company with no proprietary data becomes a $5-8B company.

2. **Traditional Aggregators (Maybe)**: Google and Meta commoditized suppliers. Now LLMs could commoditize them. But here's the nuance: only if they fail to own the LLM chat layer themselves. Google has Gemini and insane distribution. Meta has Llama. The race is on. If they win the chat interface, they stay aggregators. If they lose it, they become the commoditized.

3. **Content Creators**: UGC platforms lose relevance when AI generates personalized content. The creator economy inverts: infinite AI content, zero human creators needed for most use cases.

4. **The UI/UX Industry**: Beautiful interfaces become irrelevant when the LLM chat is the only interface. Hundreds of billions per year in frontend development... for what? Figma (amazing product!) is down by 90%.
...
1. **How much of the business is interface versus data?** Most vertical software is 60-80% interface, 20-40% data. When LLMs absorb the interface, that value evaporates.

2. **Is the data truly proprietary?** If it can be licensed, scraped, or replicated, there's no moat left. Pure commodity competition.

**This is not a bear case. This is math.**

The market hasn't priced this in because:
- LLM capabilities are new (less than 2 years at scale)
- MCP adoption is early (less than 1 year)
- Enterprise buyers move slowly (3-5 year contracts)
- Incumbents are in denial
...
│  Transaction costs → 0                    │
│  Aggregators emerge                       │
│  Suppliers keep interface + data          │
│                                           │
│  LLM ERA (2023+)                          │
│  Interface costs → 0                      │
│  Complete aggregation                     │
│  Suppliers = invisible APIs               │
│  Internet becomes a database              │
│                                           │
└───────────────────────────────────────────┘
```
...
**Related Articles:**
- Model-Market Fit (companion piece)
- The RAG Obituary: Killed by Agents, Buried by Context Windows

**Tags:** #aggregation-theory #vertical-saas #llm #mcp #interface-moat #business-model #market-dynamics #ai-disruption
