...
The durable output of this workflow is a **requirements document**. Keep the workflow name `brainstorm`, but make the written artifact strong enough that planning does not need to invent product behavior, scope boundaries, or success criteria.
...
If the user references an existing brainstorm topic or document, or there is an obvious recent matching `*-requirements.md` file in `docs/product/brainstorms/`:
- Read the document
...
*Constraint Check* — Check project instruction files (`CLAUDE.md`) for workflow, product, or scope constraints that affect the brainstorm. Also check the v2 plan and App Spec for relevant context. If these add nothing, move on.

*Topic Scan* — Search for relevant terms. Read the most relevant existing artifact if one exists (brainstorm, plan, spec, design doc). Skim adjacent examples covering similar behavior.
...
Write to `docs/product/brainstorms/YYYY-MM-DD-<topic>-requirements.md`.
...
Requirements doc: docs/product/brainstorms/YYYY-MM-DD-<topic>-requirements.md  # if one was created
...
Resume with `/brainstorm` to resolve blocking questions before planning.
```
