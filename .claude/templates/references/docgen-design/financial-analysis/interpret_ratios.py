...
Based on: https://github.com/anthropics/claude-cookbooks/tree/main/skills/custom_skills/analyzing-financial-statements

WHY RULE-BASED INTERPRETATION WORKS HERE:
- Industry benchmarks are standardized and well-documented
- Thresholds are relatively stable across time
- Context is predictable (industry + company type)
...
Usage:
    python interpret_ratios.py --input ratios.json --output analysis.json --industry technology
"""
...
class RatioInterpreter:
    ...
    # These are well-documented and relatively stable - that's why rule-based works
    BENCHMARKS = {
        "technology": {
    ...
    def interpret_ratio(self, ratio_name: str, value: float) -> dict[str, Any]:
        ...
        This is rule-based because benchmarks are standardized.
...
def main():
    """CLI entry point for ratio interpretation."""
    parser = argparse.ArgumentParser(
        description='Interpret financial ratios against industry benchmarks'
    )
    parser.add_argument(
        '--input', '-i',
        required=True,
        help='Input JSON file from calculate_ratios.py'
    )
    parser.add_argument(
        '--output', '-o',
        required=True,
        help='Output JSON file for interpretations'
    )
    parser.add_argument(
        '--industry',
        default='general',
        choices=['general', 'technology', 'retail', 'financial', 'manufacturing', 'healthcare'],
        help='Industry for benchmark selection (default: general)'
    )
    parser.add_argument(
        '--pretty', '-p',
        action='store_true',
    ...
    # Interpret ratios
    ratios = ratio_data.get('ratios', ratio_data)
    result = perform_comprehensive_analysis(ratios, industry=args.industry)
    ...
    # Print summary
    print(f"Financial Health: {result['overall_health']['status']}")
    print(f"  {result['overall_health']['message']}")
    print(f"\nTop Recommendations:")
