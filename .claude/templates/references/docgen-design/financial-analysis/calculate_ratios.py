...
Usage:
    python calculate_ratios.py --input statements.json --output ratios.json
"""
...
def main():
    """CLI entry point for ratio calculation."""
    parser = argparse.ArgumentParser(
        description='Calculate financial ratios from statement data'
    )
    parser.add_argument(
        '--input', '-i',
        required=True,
        help='Input JSON file with financial statement data'
    )
    parser.add_argument(
        '--output', '-o',
        required=True,
        help='Output JSON file for calculated ratios'
    )
    parser.add_argument(
        '--pretty', '-p',
        action='store_true',
    ...
    # Calculate ratios
    result = calculate_ratios_from_data(financial_data)
