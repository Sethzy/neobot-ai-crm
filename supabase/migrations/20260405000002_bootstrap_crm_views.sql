...
BEGIN
  ...
  -- Read configured deal stages from crm_config (the single source of truth).
  -- If the client has no config yet, deal views will use an empty stage array
  -- and the user can ask the agent to recreate them after CRM setup.
  SELECT deal_stages
    INTO v_deal_stages
    FROM public.crm_config
   WHERE client_id = p_client_id;

  -- Filter out terminal stages to build "Active pipeline"
  IF v_deal_stages IS NOT NULL THEN
    SELECT COALESCE(
  ...
  INSERT INTO public.crm_views (client_id, name, entity_type, filters, is_seeded) VALUES
    -- Tasks (always safe — status values are fixed)
    ...
    -- Contacts (type values are configurable but buyer/seller are universal defaults)
  ...
  -- Only seed deal views if we have configured stages
  IF v_active_stages IS NOT NULL AND jsonb_array_length(v_active_stages) > 0 THEN
    INSERT INTO public.crm_views (client_id, name, entity_type, filters, is_seeded) VALUES
