-- Fix: allow service_role callers (agent via Telegram, triggers, autopilot)
-- to write timeline entries. The prior migration only checked get_my_client_id(),
-- which resolves NULL for service_role JWTs since they have no auth.uid().
-- Mirror the quota RPC pattern: skip tenant check for service_role.
...
BEGIN
  -- Tenant isolation: service_role is trusted (server-side agent paths).
  -- For authenticated browser callers, p_client_id must match their tenant.
