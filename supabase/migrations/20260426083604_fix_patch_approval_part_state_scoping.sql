-- Fix patch_approval_part_state: conversation_messages has no client_id
-- column, so the original UPDATE errored with 42703 ("column 'client_id'
-- does not exist") and the calling code swallowed it. Result: every
-- resolved approval stayed persisted as state='approval-requested', so
-- the approval card kept rendering Allow/Deny on reload after the user
-- had already decided.
--
-- Tenant scoping for conversation_messages goes through the thread row.
...
BEGIN
  ...
  IF v_thread_owner IS DISTINCT FROM p_client_id THEN
    RETURN jsonb_build_object('status', 'missing', 'event', NULL);
  END IF;
...
-- Backfill: every approval already resolved before this fix landed left
-- its persisted part stuck on state='approval-requested'. Walk the
-- resolved approvals and patch the matching parts in place.
DO $$
DECLARE
  v_event RECORD;
  v_approved BOOLEAN;
BEGIN
  FOR v_event IN
    SELECT approval_id, thread_id, status
    FROM public.approval_events
  ...
  LOOP
    ...
    SET parts = (
      SELECT jsonb_agg(
        CASE
          WHEN elem->'approval'->>'id' = v_event.approval_id
            AND elem->>'state' IN ('approval-requested', 'approval-responded')
          THEN jsonb_set(
            jsonb_set(elem, '{state}', '"approval-responded"'::jsonb),
            '{approval,approved}',
    ...
    WHERE thread_id = v_event.thread_id
      AND role = 'assistant'
      AND parts IS NOT NULL
      AND EXISTS (
        SELECT 1
        FROM jsonb_array_elements(parts) AS elem
        WHERE elem->'approval'->>'id' = v_event.approval_id
          AND elem->>'state' = 'approval-requested'
      );
  END LOOP;
END $$;
